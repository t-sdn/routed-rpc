...
class HelloSouthboundExporter <T extends DataObject> implements TsdnInventoryService {

	private static final Logger LOG = LoggerFactory.getLogger(HelloSouthboundExporter.class);
	
	private final HelloSouthboundOperationProcessor processor;
	private final NotificationProviderService notiService;
	
	HelloSouthboundExporter (ProviderContext session, HelloSouthboundOperationProcessor processor, NotificationProviderService notiService) {
	...
	public Future<RpcResult<SetTunnelOutput>> setTunnel(SetTunnelInput input) {
		...
		LOG.info("HelloSBExporter is called");
